...
function makesMill(board, player, lastMove) {
  ...
  if (pairs[posIndex].some(pair => square[pair[0]] === player && square[pair[1]] === player)) {
    return true;
  }
  ...
  if(board.length >= 3){
    if(squareIndex === 0){
      if(board[1][posIndex] === player && board[2][posIndex] === player) return true;
    }
    if(squareIndex === board.length - 1){
      if(board[board.length - 2][posIndex] === player && board[board.length - 3][posIndex] === player) return true;
    }
    if(board[squareIndex + 1][posIndex] === player && board[squareIndex - 1][posIndex] === player) return true;
  }
